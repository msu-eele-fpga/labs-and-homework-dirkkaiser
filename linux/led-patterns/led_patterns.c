...
#include <linux/io.h> // iowrite32/ioread32 functions are in this lib
 ...
 * 
 * An led_patterns_dev struct gets created for each eld patterns componendt.
 */
 ...
 *	 device struct is embedded in the led_patters' platform
 *	 device struct.
 * @attr: Unused.
 ...
 * @dev: Device structure for the led_patterns component. This device struct is embedded in the 
 * 	 led_patterns' platform device struct.
 * @attr: Unused.
...
/**
 * hps_led_control_show() - Return the hps_led_control value to user-space via sysfs.
 *
 * @dev: Device sturcture for the led_Patterns component. THis device struct is embedded in the led_patterns' device struct.
 * @attr: unused.
 * @buf: Buffer that gers returned to user-space.
 *
 * Return: The number of bytes read.
 */
static ssize_t hps_led_control_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	// Get the provate led_patterns data out of the dev struct
	struct led_patterns_dev *priv = dev_get_drvdata(dev);
 ...
 * @dev: Device structure for the led_patterns component. 
 * 	device struct is embbed in the led_patterns' platform device struct.
 * @attr: unused
 ...
 * @dev: Device structrue for the led_patterns component. This device struct is embedded in the platfrom struct
 * @attr: unused again
 * @buf: buffer that gets returned to user-space.
 *
 * Return: the number of bytes read.
 ...
 * @dev: Device structure fo the led_patterns component. THis device struct is embeedded in the platform struct
 * @attr: Unsused.
 * @buf: Buffer that contains the base_period value being written.
 * @sizeL the number of bytes being written.
 * 
 * Return: the number of bytes stored.
 */
...
/*
 * led_patterns_read() - Read emthod for the led_patterns char device
 * @file: Pointer to the char device file struct.
 * @buf: User-space buffer to read the value into.
 * @count: The number of bytes being requested.
 * @offset: The byte offset in the file being read from.
 *
 * Return: On success, the number ofbytes written is returned an the
 * offset @offset is advanced by this number. ON erro, a negative error
 * value is returned.
 */
	...
	size_t count, loff_t *offset)
{
	 ...
	 * led_patterns_dev struct. contaer _of returns the led_patterns_dev
...
/*
 * I think my whole problem with this class is that we never actually have to think about anything.
 * we spend so much time complaining how the previous classes are bad because they spoon feed us
 * but this class is doing exactly the same thing. Like I am literaly just copying code right now.
 * The problem is it so hard to teach any of this because its so complicated and like
 * said braindead hardware and braindead software that we would never be able to figure this out on our own.
 * That said there is some learning in doing but I really feal like it is an uneffective way to learn.
 ...
 * @file: Pointer to the char device file struct.
 * @vuf: User-space buffer to read the value from.
 * @count: The number of bytes being written.
 * @offset: The byte offset in the file being written to.
 *
 * Return: On success, the number of bytes written is returned an the 
	...
	size_t count, loff_t *offset)
{
	...
	if ((*offset % 0x4) != 0) {
		pr_warn("led_patterns_write: unaligned access idiot\n");
		return -EFAULT;
	}
 ...
 * 	   that the driver can't be removed while the character device is still in use.
 * @read: The read function.
 * @write: The write function.
 * @llseek: We use the kernel's default_llseek() function; this allows
 *	   users to change what positions they are writing/reading to/from.
 */
 ...
 *	 led pattterns device tree node.
 *
 * When a device that is compatible with this led patterns driver is found,
 * the driver's probe function is called. This probe function gets called by
 * the kernel when an led_patterns device is found it the device tree.
 */
...
static int led_patterns_probe(struct platform_device *pdev)
{
	...
	priv = devm_kzalloc(&pdev->dev, sizeof(struct led_patterns_dev),
			    GFP_KERNEL);
	if (!priv) {
		pr_err("You are such an idiot: Failed to allocate memory\n");
		return -ENOMEM;
	}
	 ...
	 * make sure nobody else can use that memory. The memory is remapped 
 	 * into the kernel's virtual addres space because we don't have access
	...
	// Register the misc device; theis creates a char dev at /dev/led_patterns
	...
	/* Attach the led patterns' private data to the platform device's struc.
 ...
 * @pdev: Platform device structure associated with our led patterns device.
 * 
 * THis function is called when an led patetterns device is removed or 
 * the driver is removed.
 */

static int led_patterns_remove(struct platform_device *pdev)
{
	// Get the led patterns' private data from the platform device.
	struct led_patterns_dev *priv = platform_get_drvdata(pdev);

	// Disable software-control mode, just for shits and giggles.
...
/*
 * Define the compatible property used for matching devices to thsi driver,
...
static const struct of_device_id led_patterns_of_match[] = {
	{ .compatible = "Kaiser,led_patterns", },
	{ }
};
 ...
 * This macro artomatically handles module inint/exit.
 */
...
MODULE_AUTHOR("Dirk");
MODULE_DESCRIPTION("led_patterns");
